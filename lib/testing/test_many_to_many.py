...
def test_contract_contracts_by_date():
    ...
    contracts_sorted = Contract.contracts_by_date('01/01/2001')
    assert all(contracts_sorted[i].date <= contracts_sorted[i + 1].date for i in range(len(contracts_sorted) - 1))
